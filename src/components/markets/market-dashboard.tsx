...
import PolymarketMiniChart from "@/components/markets/mini-chart"
...
const categoryFilters = [
  ...
  {id:"finance",label:"Finance", icon: DollarSign,isFilter:false },
  { id: 'earnings', label: "Earnings", icon: HandCoins, isFilter: false },
  {id:"economy",label:'Economy',icon: CircleDollarSign, isFilter:false},
  {id:"elections",label:'Elections',icon: Vote,isFilter:false },
...
{id:'world',label:"World",icon: Globe,isFilter:false },
...
export default function MarketDashboard({
  ...
  return (
    <div className="relative " id="markets">
         <div className="bg-primary absolute -top-10 left-1/2 h-16 w-44 -translate-x-1/2 rounded-full opacity-40 blur-3xl select-none"></div>
                <div className="via-primary/50 absolute top-0 left-1/2 h-px w-3/5 -translate-x-1/2  bg-gradient-to-r from-transparent to-transparent transition-all ease-in-out"></div>
                <h2
                    className={cn(
                        "via-foreground mb-8 bg-gradient-to-b mt-10 from-zinc-800 to-zinc-700 bg-clip-text text-center text-4xl font-semibold tracking-tighter text-transparent md:text-[54px] md:leading-[60px]",
                        geist.className,
                    )}
                >
                    Markets
      ...
      <div className="min-h-screen text-foreground border-10  bg-background rounded-xl">
        {/* Header with Search and Filters */}
        <motion.header
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="border-b border-border  backdrop-blur-sm sticky top-0  "
        >
