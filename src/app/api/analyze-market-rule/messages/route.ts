...
export async function POST(req: Request) {
 ...
 const session = await auth.api.getSession({
    ...
    if (conversationId) {
      conversation = await prisma.rulesConversation.findUnique({
      ...
      conversation = await prisma.rulesConversation.create({
        data: {
          userId: user.id,
    ...
    const savedMessage = await prisma.rulesMessage.create({
      data: {
    ...
    await prisma.rulesConversation.update({
