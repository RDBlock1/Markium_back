...
export function useRuleChatHistory() {
  ...
  const fetchConversations = async () => {
    try {
      setIsLoading(true)
      const response = await fetch('/api/ai-rules-conversations')
  ...
  const saveMessage = async (message: Message, markets?: any[]) => {
    ...
    try {
      ...
      const response = await fetch('/api/analyze-market-rule/messages', {
        method: 'POST',
  ...
  const loadChat = useCallback(
    async (chatId: string) => {
      try {
        ...
        const response = await fetch(`/api/ai-rules-conversations/${chatId}`)
  ...
  const renameChat = useCallback(async (chatId: string, newTitle: string) => {
    try {
      const response = await fetch(`/api/ai-rules-conversations/${chatId}/rename`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const favoriteChat = useCallback(async (chatId: string) => {
    try {
      ...
      const response = await fetch(`/api/ai-rules-conversations/${chatId}/favorite`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
